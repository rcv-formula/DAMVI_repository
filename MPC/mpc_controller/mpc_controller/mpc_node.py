...
class MPCController(Node):
    def __init__(self):
        ...
        self.create_subscription(Float32MultiArray, '/local_costmap', self.cost_map_callback, 10)
    ...
    def cost_map_callback(self, msg):
        ...
        size = int(np.sqrt(data_length))
    
        # Validate if the data length forms a square grid
        if size * size != data_length:
            self.get_logger().warn(f"Invalid cost map data length: {data_length}. Expected a square grid.")
            return
    
        # Reshape the cost map
        self.cost_map = np.array(msg.data).reshape((size, size))
        self.get_logger().info(f"Cost map updated with size: {size}x{size}")
    ...
    def generate_local_trajectory(self, closest_idx):
        ...
        if len(self.global_path) - closest_idx < self.horizon + 1:
    ...
    def publish_local_path(self, local_path):
        ...
        self.get_logger().info(f"Published local path with {len(local_path)} points.")
    ...
    def map_cost(self, x, y):
        ...
        # Check bounds
        if 0 <= grid_x < self.cost_map.shape[0] and 0 <= grid_y < self.cost_map.shape[1]:
            return self.cost_map[grid_y, grid_x]
        else:
            self.get_logger().warn(f"Position ({x:.2f}, {y:.2f}) is outside cost map bounds.")
    ...
    def run_mpc(self):
        ...
        if prob.status == cp.OPTIMAL:
            ...
            self.control = self.control if self.control else [0.0, 0.0]  # Reset to zero if undefined
        ...
        self.get_logger().info(f"Solver status: {prob.status}")
        self.get_logger().info(f"Solver iterations: {prob.solver_stats.num_iters}")
        self.get_logger().info(f"Solver time: {prob.solver_stats.solve_time:.4f}s")
        self.get_logger().info(f"Final cost: {prob.value:.4f}")
    ...
    def publish_cmd_drive(self):
        ...
        self.get_logger().info(f"Published drive command: speed={self.control[0]:.2f}, angle={self.control[1]:.2f}")
